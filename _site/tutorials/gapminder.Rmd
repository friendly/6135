...
The data has conviently been packaged in the R package [gapminder](https://CRAN.R-project.org/package=gapminder),
...
To make such plots prettier, you can change the line thickness (`size=`), add a fill color (`fill=""`),
and make the fill color partially transparent (`alpha=`). You can see the grid lines underneath the 
following plots
...
If you want, you can also add a histogram later.  This is a little more complicated to get right, because
historams are computed differently and need some additional arguments.
...
I don't recommend such plots, because the density plot usually gives most of the information, and
...
![gapminder animation using plotly](../images/gapminder-highlight-animation.gif)
